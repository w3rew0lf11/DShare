    ...
    try:
        ...
        with data_lock:
            ...
            if agent_name not in active_agents:
                is_new_agent = True
                logger.info(f"New agent connected: {agent_name}")
            
            agent_data[agent_name] = data
            active_agents.add(agent_name)
            
            # Emit system stats as usual
            socketio.emit('system_stats', {
                'agent_name': agent_name,
                'data': data
            }, namespace='/')
            
            # If this is a new agent, emit an agent_connected event
            if is_new_agent:
...
def background_agent_checker():
    """Clean up inactive agents and emit disconnection events"""
    while True:
        time.sleep(15)
        ...
        with data_lock:
            ...
            # Emit disconnection events before removing agents
            if to_remove:
                socketio.emit('agent_disconnected', {
                    'agents': list(to_remove)
                })
                logger.info(f"Emitted disconnection event for agents: {to_remove}")
            
            # Remove the agents after emitting events
